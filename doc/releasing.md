...
- pub.dev: リポジトリ`LibraryLibrarian/misskey_emoji`、タグ`v{{version}}`、pushイベント、Environment `pub.dev`を指定します。
...
3. 生成されたmain向けPRを確認します。pubspec、READMEの2ファイル、CHANGELOGの日付付き見出しが更新され、実行者がAssigneeになります。
...
`Tag Release`はバージョンを`pubspec.yaml`から直接読みます。このパッケージはnative assetsのビルドフックを持ち、`dart run`の進捗が標準出力へ混ざるためです。同じ理由で`tool/release_notes.dart`は出力先をリダイレクトではなく引数で受け取ります。

## プレリリースから正式版への昇格

`2.0.0-beta.1`のようなプレリリースを`2.0.0`として公開する場合も、手順は通常のリリースと同じです。`Prepare Release`のチャンネルに`stable`を選んでください。バージョンは直近の安定版に影響度を適用して決まります。開いているプレリリース列に後から破壊的変更が入った場合は、列のコアより大きい値になります。

このとき`tool/bump_version.dart`は昇格モードで動作します。

- `[Unreleased]`の内容と、直近の安定版より後の全プレリリース（`2.0.0-beta.1`など）の内容を、カテゴリ別に統合して`## [2.0.0]`に書き出します
...
- 完全に同一の行は重複排除されます
- 統合されたプレリリースの見出しと日付は残り、本文は`Included in [2.0.0].`に置き換わります
