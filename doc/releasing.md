...
2. Actionsの`Prepare Release`をdevelopから起動し、未公開のバージョンを入力します。例: `2.1.0-beta.1`。
...
`2.0.0-beta.1`のようなプレリリースを`2.0.0`として公開する場合も、手順は通常のリリースと同じです。`Prepare Release`に`2.0.0`を入力してください。

このとき`tool/bump_version.dart`は昇格モードで動作します。

- `[Unreleased]`の内容と、同じベースを持つ全プレリリース（`2.0.0-beta.1`など）の内容を、カテゴリ別に統合して`## [2.0.0]`に書き出します
