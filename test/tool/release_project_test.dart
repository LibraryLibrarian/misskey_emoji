...
void main() {
  ...
  setUp(() {
    root = Directory.systemTemp.createTempSync('misskey_emoji_release_tools_');
    _createProject(root);
  });
  ...
  test('bumpVersion updates all version references and CHANGELOG', () {
    ...
    for (final path in versionReferencePaths) {
      expect(
        _read(root, path),
        contains('misskey_emoji: ^1.0.0-beta.4'),
        reason: path,
      );
    }
    expect(
