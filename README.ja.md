...
この開発版にはDart `>=3.10.0 <4.0.0`およびFlutter `>=3.38.1`が必要です。Flutter 3.38.0の同梱Dartは3.10.0-290.4.betaで、プレリリースは`>=3.10.0`を満たしません。Dart 3.10.0の正式版を積むのはFlutter 3.38.1からです。
...
実測では`.isar`ファイルは初期状態で1 MiBが事前確保されていました。対応する`.isar-lck`ファイルとともに旧ストアを閉じた後も残りますが、ロックファイルのサイズはclose時に変わることがあります。削除前には必ず`misskey_emoji_`プレフィックスで絞り込んでください。アプリケーションが同じディレクトリで独自のIsarデータベースを使っている場合、それらを削除してはいけません。これらは再取得可能なキャッシュなので、移行のために古い依存を維持したり、旧ファイル名の規約をライブラリに固定したりはしません。
